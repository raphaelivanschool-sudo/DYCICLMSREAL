...
export default function GradeScaleManager() {
  ...
  const cell = 'w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1.5 text-sm font-mono text-center focus:ring-2 focus:ring-blue-500 focus:outline-none';
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Transmutation scale</h3>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">Maps a weighted percentage to the 1.00–5.00 grade point. 3.00 is the lowest passing (75%); below 75% is 5.00; there is no 4.00.</p>
        </div>
      </div>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} className="border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                  ...
                  <td className="px-4 py-2"><input className="w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" value={r.label} onChange={(e) => update(i, 'label', e.target.value)} /></td>
                  <td className="px-4 py-2 text-right"><button onClick={() => removeRow(i)} className="text-zinc-400 hover:text-red-500"><Trash2 size={15} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => { setRows(DEFAULT_ROWS.map((r) => ({ ...r, label: r.label || '' }))); setSaved(false); }} className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-700 px-4 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700">
          <RotateCcw size={15} /> Reset to default
        </button>
        {saved && <span className="inline-flex items-center gap-1.5 text-sm text-emerald-600 dark:text-emerald-400"><CheckCircle2 size={15} /> Saved</span>}
      </div>
    </div>
  );
}
