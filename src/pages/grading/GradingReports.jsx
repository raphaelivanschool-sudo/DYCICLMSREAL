...
export default function GradingReports() {
  ...
  const selectClass = 'bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none';
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading && <div className="animate-pulse bg-gray-100 rounded-2xl h-40" />}

      {semReport && !loading && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard label="Total grades" value={semReport.summary.total} />
            <StatCard label="Passed" value={semReport.summary.passed} color="text-emerald-600" />
            <StatCard label="Failed" value={semReport.summary.failed} color="text-red-600" />
            <StatCard label="INC" value={semReport.summary.inc} color="text-amber-600" />
          </div>

          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200">
              <h3 className="text-sm font-semibold text-gray-700">Per-student GWA</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['Student', 'Subjects', 'INC', 'GWA'].map((h) => <th key={h} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-400">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {semReport.students.length === 0 ? (
                  <tr><td colSpan={4} className="px-5 py-10 text-center text-sm text-gray-400">No enrollments in this semester.</td></tr>
                  ...
                  <tr key={s.studentId} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="px-5 py-3 text-gray-500">{s.subjects}</td>
                    <td className="px-5 py-3 text-gray-500">{s.inc}</td>
          ...
          {secReport && (
            <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200">
                <div>
                  <h3 className="text-sm font-semibold text-gray-700">{secReport.section.subject.code} — {secReport.section.name}</h3>
                  <p className="text-xs text-gray-400 mt-0.5">{secReport.section.instructor} · {secReport.summary.passed} passed · {secReport.summary.failed} failed · {secReport.summary.inc} INC</p>
                </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    {['Student', 'Percentage', 'Grade', 'Remark', 'Status'].map((h) => <th key={h} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-400">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {secReport.rows.length === 0 ? (
                    <tr><td colSpan={5} className="px-5 py-10 text-center text-sm text-gray-400">No students enrolled.</td></tr>
                  ) : secReport.rows.map((r) => (
                    <tr key={r.studentId} className="border-b border-gray-100 last:border-0">
                      <td className="px-5 py-3 font-medium text-gray-900">{r.student}</td>
                      <td className="px-5 py-3 font-mono text-gray-500">{fmtPct(r.percentage)}</td>
