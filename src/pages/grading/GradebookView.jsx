...
export default function GradebookView({ sectionId, readOnly = false, onSubmitted }) {
  ...
  if (loading) return <div className="animate-pulse bg-zinc-100 dark:bg-zinc-800 rounded-2xl h-64" />;
  ...
  const cellInput = `w-16 text-center rounded-lg px-1.5 py-1 text-sm font-mono bg-zinc-50 dark:bg-zinc-800
    border border-zinc-200 dark:border-zinc-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Stat icon={<Users size={15} className="text-zinc-400" />} label="Students" value={data.analytics.total} />
        <Stat icon={<TrendingUp size={15} className="text-zinc-400" />} label="Class avg" value={fmtPct(data.analytics.classAverage)} />
        <Stat icon={<AlertTriangle size={15} className={data.analytics.failing > 0 ? 'text-red-500' : 'text-zinc-400'} />} label="Failing" value={data.analytics.failing} danger={data.analytics.failing > 0} />
        <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium ${weightOk ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400' : 'bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400'}`}>
        ...
        {!editable && !readOnly && (
          <span className="ml-auto inline-flex items-center gap-1.5 text-sm text-zinc-500"><Lock size={14} /> Read-only ({STATUS_LABEL[sectionStatus]})</span>
        )}
      </div>
      ...
      {editable && (
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-200">Categories &amp; activities</h3>
            <button onClick={() => setCatModal(true)} className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 px-3 py-1.5 rounded-xl border border-blue-200 dark:border-blue-800 transition-colors">
              <Plus size={15} /> Add category
            </button>
          </div>
          {data.categories.length === 0 ? (
            <p className="text-sm text-zinc-400">No categories yet. Add Class Standing, Quizzes, Major Exam, etc. (weights must total 100%).</p>
          ) : (
            <div className="flex flex-col gap-3">
              {data.categories.map((cat) => (
                <div key={cat.id} className="border border-zinc-200 dark:border-zinc-800 rounded-xl p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-zinc-800 dark:text-zinc-100">{cat.name}</span>
                      <span className="text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded-lg">{cat.weight}%</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => removeCategory(cat.id)} className="text-zinc-400 hover:text-red-500"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  {cat.activities.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {cat.activities.map((a) => (
                        <span key={a.id} className="inline-flex items-center gap-1.5 text-xs bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1">
                          {a.title} <span className="text-zinc-400">/{a.maxScore}</span>
                          <button onClick={() => removeActivity(a.id)} className="text-zinc-400 hover:text-red-500"><Trash2 size={12} /></button>
                        </span>
                      ))}
                    </div>
                  )}
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-800/60 border-b border-zinc-200 dark:border-zinc-800">
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-400 sticky left-0 bg-zinc-50 dark:bg-zinc-800/60 min-w-[180px]">Student</th>
                {data.categories.map((cat) => cat.activities.map((a) => (
                  <th key={a.id} className="px-3 py-3 text-center text-xs font-medium text-zinc-500 whitespace-nowrap" title={`${cat.name} • max ${a.maxScore}`}>
                    {a.title}<div className="text-[10px] text-zinc-400 font-normal">/{a.maxScore}</div>
                  </th>
                )))}
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-zinc-400">Running %</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-zinc-400">Grade</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-zinc-400">Remark</th>
                {editable && <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-zinc-400">INC</th>}
              </tr>
            </thead>
            <tbody>
              {students.length === 0 ? (
                <tr><td colSpan={99} className="px-4 py-12 text-center text-sm text-zinc-400">No students enrolled in this section yet.</td></tr>
              ) : students.map((s) => (
                <tr key={s.enrollmentId} className="border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                  <td className="px-4 py-3 sticky left-0 bg-white dark:bg-zinc-900 min-w-[180px]">
                    <p className="font-medium text-zinc-900 dark:text-zinc-100">{s.student.fullName}</p>
                      ...
                      <td key={a.id} className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          {editable ? (
                            ...
                            <span className="font-mono text-zinc-700 dark:text-zinc-200">{scores[key] ?? '—'}</span>
                          )}
                          {savingCell === key && <Loader2 size={12} className="animate-spin text-blue-500" />}
                          {(editable || hasFb) && (
                            <button onClick={() => setFbModal({ activityId: a.id, enrollmentId: s.enrollmentId, current: fbFor(a.id, s.enrollmentId) })} className={hasFb ? 'text-blue-500' : 'text-zinc-300 hover:text-zinc-500'} title={hasFb ? 'View/edit feedback' : 'Add feedback'} disabled={!editable && !hasFb}>
                  ...
                  <td className="px-4 py-3 text-center font-mono text-zinc-600 dark:text-zinc-300">{fmtPct(s.runningPercentage)}</td>
                  ...
                  {editable && (
                    <td className="px-4 py-3 text-center">
                      {s.isInc
                        ...
                        : <button onClick={() => markInc(s.enrollmentId)} className="text-xs text-zinc-400 hover:text-amber-600">Mark INC</button>}
                    </td>
                  )}
                </tr>
              ))}
      ...
      {submitModal && (
        <Modal onClose={() => setSubmitModal(false)} title="Submit final grades">
          ...
          <div className="flex gap-3 justify-end mt-4">
            <button onClick={() => setSubmitModal(false)} className="px-4 py-2 rounded-xl text-sm font-medium border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800">Cancel</button>
...
function Stat({ icon, label, value, danger }) {
  return (
    <div className="inline-flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-3 py-2 rounded-xl text-sm">
      {icon}
      <span className="text-zinc-400">{label}:</span>
      <span className={`font-semibold ${danger ? 'text-red-600 dark:text-red-400' : 'text-zinc-800 dark:text-zinc-100'}`}>{value}</span>
    </div>
  );
}
...
function CategoryModal({ onClose, onSave, busy }) {
  ...
  const inputClass = 'w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none';
  return (
    <Modal onClose={onClose} title="New category">
      <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="flex flex-col gap-4">
        ...
        <div className="flex gap-3 justify-end mt-2">
          <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl text-sm font-medium border border-zinc-200 dark:border-zinc-700">Cancel</button>
...
function FeedbackModal({ initial, onClose, onSave, busy, readOnly }) {
  ...
  return (
    <Modal onClose={onClose} title={readOnly ? 'Feedback' : 'Score feedback'}>
      {readOnly ? (
        <p className="text-sm text-zinc-600 dark:text-zinc-300 whitespace-pre-wrap">{text || 'No feedback.'}</p>
      ) : (
        <>
          <textarea rows={4} placeholder="Feedback for this score..." className={inputClass} value={text} onChange={(e) => setText(e.target.value)} />
          <div className="flex gap-3 justify-end mt-4">
            <button onClick={onClose} className="px-4 py-2 rounded-xl text-sm font-medium border border-zinc-200 dark:border-zinc-700">Cancel</button>
